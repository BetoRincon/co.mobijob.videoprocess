...
#define TAG "co.mobijob.videoprocess"

using namespace v8;

static Persistent<Object> bindingCache;

static void CoMobijobVideoprocess_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "CoMobijobVideoprocess.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::CoMobijobVideoprocessBindings::lookupGeneratedInit(
...
static void CoMobijobVideoprocess_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, CoMobijobVideoprocess_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void CoMobijobVideoprocess_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::CoMobijobVideoprocessBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry CoMobijobVideoprocessBinding = {
	"co.mobijob.videoprocess",
	CoMobijobVideoprocess_init,
	CoMobijobVideoprocess_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_co_mobijob_videoprocess_CoMobijobVideoprocessBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("co.mobijob.videoprocess", &CoMobijobVideoprocessBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::CoMobijobVideoprocessBindings::lookupGeneratedInit));
}
